...
/*[双核共享内存]******************************************/
/*======================================================*/
// CM7_1 写 yaw / 读命令，CM7_0 读 yaw / 写命令
// 地址 0x280B8000 位于 CM7_1 SRAM 区域内，两个核通过 AHB 总线都能访问
typedef struct {
    volatile int16  yaw;         // CM7_1 -> CM7_0：yaw 值（0.01°/LSB）
    volatile uint8  cmd_calib;   // CM7_0 -> CM7_1：1 = 触发零漂校准
    volatile uint8  cmd_reset;   // CM7_0 -> CM7_1：1 = 触发 Yaw 归零
} shared_imu_t;

#define SHARED_IMU_ADDR    ((volatile shared_imu_t *)0x280B8000)

// cache 同步（共享内存 32 字节对齐，覆盖整个结构体；CM7 D-cache 使能时必需）
#define SHARED_IMU_WRITE_SYNC()    SCB_CleanDCache_by_Addr((uint32_t*)SHARED_IMU_ADDR, 32)
#define SHARED_IMU_READ_SYNC()     SCB_InvalidateDCache_by_Addr((uint32_t*)SHARED_IMU_ADDR, 32)
/*======================================================*/
/******************************************[双核共享内存]*/
/*======================================================*/
